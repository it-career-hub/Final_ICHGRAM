  ...
  return (
    <div className={s.registerFormBox}>
      <form className={s.registerForm} onSubmit={handleSubmit}>
        ...
        />
        {error.email && <p className={s.errorMessage}>{error.email}</p>} {/* Сообщение об ошибке email */}
        ...
        <CustomInput
          ...
          style={{ paddingLeft: '8px', backgroundColor: '#FAFAFA', color: '#737373', marginTop: '6px' }}
        />
        {error.username && <p className={s.errorMessage}>{error.username}</p>} {/* Сообщение об ошибке username */}
        ...
        {error.general && <p className={s.errorMessage}>{error.general}</p>} {/* Общее сообщение об ошибке */}
        ...
        <p className={s.registerForm_p2}>
          ...
          <span className={s.agreementLink}>{t('registerForm.privacyPolicy')}</span>,{' '}
          <span className={s.agreementLink}>{t('registerForm.cookiesPolicy')}</span>.
        </p>
        <CustomButton style={{ width: '268px', height: '32px' }} text={t('registerForm.signUpButton')} type="submit" />
      </form>
      <div className={s.haveAccountBox}>
        <p>
          {t('registerForm.haveAccount')}{' '}
          <Link to={'/'} style={{ color: "var(--color-text-blue)", fontWeight: 600 }}>{t('registerForm.login')}</Link>
        </p>
      </div>
    </div>
  );
