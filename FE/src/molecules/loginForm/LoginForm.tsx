...
>
  {showPassword ? <HiOutlineEyeOff /> : <HiOutlineEye />}
        ...
        <Link to={'/reset'} className={s.forgotPasswordLink}>{t('loginForm.forgotPassword')}</Link>
      </form>
      <div className={s.haveAccountBox}>
        <p>{t('loginForm.haveAccount')} <Link to={'/register'} style={{ color: 'var(--color-text-blue)', fontWeight: 600 }}>{t('loginForm.sign')}</Link></p>
      </div>
    </div>
  );
};
